...
class ScoringService:
    ...
    def _skill_match_score(self, jd_skills, resume_skills):
        ...
        jd_cats = {map_skill_to_category(normalize_skill(s)) for s in jd_skills}
        resume_cats = {map_skill_to_category(normalize_skill(s)) for s in resume_skills}

        # Category match
        matched = len(jd_cats.intersection(resume_cats))
        total = max(len(jd_cats), 1)
